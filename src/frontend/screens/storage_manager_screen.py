...
from frontend.screens.storage_location_screen import StorageLocationScreen
...
class StorageManagerPage(BoxLayout):
    ...
    def generate_buttons(self, titles: List[str]) -> List[ButtonModel]:
        """Generate buttons for each storage location."""
        btns = []
        for title in titles:
    ...
    def generate_screens(self):
        """Create or update screens for each storage location."""
        sm = App.get_running_app().sm
        inventory_manager = App.get_running_app().inventory_manager
        
        for name in self.locations:
            _name = name.lower()
            
            if not sm.has_screen(_name):
                screen = StorageLocationScreen(_name, inventory_manager)
                sm.add_widget(screen)
                print(f"Created screen for: {name}")
            else:
                screen = sm.get_screen(_name)
                if hasattr(screen, 'page'):
                    screen.page.location_name = name
                print(f"Updated screen for: {name}")

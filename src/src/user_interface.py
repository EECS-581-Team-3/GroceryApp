...
from back_button import BackButtonModel
...
class PantryPage(BoxLayout):
    ...
    def build_layout(self):
        ...
        back_btn = BackButtonModel(callback=self.go_back)
        home_btn = HomeButtonModel(callback=self.go_home)
        edit_btn = EditButtonModel(callback=self.go_edit)
...
class FridgePage(BoxLayout):
    ...
    def refresh_items(self):
        ...
        all_items = self.inventory_manager.all_items()
        fridge_items = [item for item in all_items if getattr(item, 'location', None) == 'Fridge']

        fridge_items.sort(key=lambda x: x.name.lower())
...
class FridgeScreen(Screen):
    def __init__(self, inventory_manager: InventoryManager, **kwargs):
        super().__init__(name='fridge', **kwargs)
        self.page = FridgePage(inventory_manager)
        self.add_widget(self.page)
